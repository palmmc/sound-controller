...
import com.bvengo.soundcontroller.platform.Services;
...
public class ConfigParser {
	private static final File file = new File(Services.PLATFORM.getConfigFolder().toFile(),
	...
	private static JsonObject createJsonConfig(VolumeConfig config) {
		...
		JsonArray disabledCats = new JsonArray();
		config.getDisabledCategories().stream().sorted().forEach(disabledCats::add);
		jsonObject.add("disabledCategories", disabledCats);
...
//		}
	...
	private static void parseConfig4(VolumeConfig config, JsonObject jsonObject) {
		...
		JsonArray disabledCats = jsonObject.getAsJsonArray("disabledCategories");
		config.getDisabledCategories().clear();
		if (disabledCats != null) {
			for (JsonElement el : disabledCats) {
				config.getDisabledCategories().add(el.getAsString());
			}
		}
